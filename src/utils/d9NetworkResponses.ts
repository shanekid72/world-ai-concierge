...
// Utility function to get country information from the network data
export const getCountryData = (country: string) => {
  // Use a safe approach with optional chaining to avoid errors
  return d9NetworkData?.countries?.[country?.toLowerCase()] || null;
};
...
// Exporting the original function for backwards compatibility
export const d9NetworkResponses = {
  getCountryServiceResponse,
};
